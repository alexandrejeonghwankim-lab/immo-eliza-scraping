...
file_path = os.path.join(data_dir, "sale_properties.csv") 

HTML_DIR = new_folder_dir
...
def normalize_category(prop_type):
    ...
    apt_types = {"apartment", "duplex", "penthouse", "studio", "flat", "loft","groundFloor"}
...
def extract_property(filepath):
    ...
    if main_content:
        epc_tag = main_content.find("p", class_="energy-consumption")
        if epc_tag:
            epc_classes = epc_tag.get("class", [])
            for cls in epc_classes:
                if cls != "energy-consumption":
                    data["epc"] = cls
                    break
    ...
    for js in jsonlds:
        ...
        elif obj_type == "RealEstateListing":
            data["posting_date"] = js.get("datePosted").split("T")[0]
    ...
    state_pattern = r'Building condition|State.*? (Normal|To renovate|New|Good|As new|Just renovated|to-be-renovated)'
    ...
    vat_match = re.search(r'VAT??\s*(Yes|No)', text, flags=re.I)
    if vat_match:
    ...
    epc_match = re.search(r'EPC *? ([A-G\+\-]+)', text, flags=re.I)
